...
class SSD1306_SoftI2C(SSD1306):
  def __init__(self, width, height, SoftI2C, addr=0x3c, external_vcc=False):
    self.SoftI2C = SoftI2C
    self.addr = addr
  ...
  def write_cmd(self, cmd):
    ...
    if currentBoard=="esp8266" or currentBoard=="esp32":
      self.SoftI2C.writeto(self.addr, self.temp)
    elif currentBoard=="pyboard":
      self.SoftI2C.send(self.temp,self.addr)
    #ELSE:
  ...
  def write_data(self, buf):
    ...
    if currentBoard=="esp8266" or currentBoard=="esp32":
      self.SoftI2C.start()
      self.SoftI2C.write(self.temp)
      self.SoftI2C.write(buf)
      self.SoftI2C.stop()
    elif currentBoard=="pyboard":
      #self.SoftI2C.send(self.temp,self.addr)
      #self.SoftI2C.send(buf,self.addr)
      self.SoftI2C.mem_write(buf,self.addr,0x40)
  def poweron(self):
    pass
...
oled.text('Olay', 0, 10)
oled.text('hsadasdasdasdas', 0, 20)
oled.text('sadqwnasjchjqwdna5151d5q1eds', 0, 30)
